...
def create_text_file_contents_manager(cm_class):
    """Return a contents manager that derives from the given CM class"""

    class _TextFileContentsManager(cm_class, Configurable):
        """
        ...
        def save(self, model, path=''):
            ...
            if model['type'] != 'notebook':
                return super(_TextFileContentsManager, self).save(model, path)
            ...
            try:
                ...
                jupytext_formats = [preferred_format(fmt, self.preferred_jupytext_formats_save) for fmt in
                                    jupytext_formats]
                ...
                for fmt in jupytext_formats[::-1]:
                    ...
                    value = super(_TextFileContentsManager, self).save(model, alt_path)
                    if alt_path == path:
                        return_value = value
        ...
        def get(self, path, content=True, type=None, format=None, load_alternative_format=True):
            ...
            if (not self.exists(path) or os.path.isdir(os_path) or
                ...
                return super(_TextFileContentsManager, self).get(path, content, type, format)
        ...
        def rename_file(self, old_path, new_path):
            ...
            if old_path not in self.paired_notebooks:
                super(_TextFileContentsManager, self).rename_file(old_path, new_path)
                return
            ...
            for old_alt_path, alt_fmt in old_alt_paths:
                ...
                if self.exists(old_alt_path):
                    super(_TextFileContentsManager, self).rename_file(old_alt_path, new_alt_path)

            self.drop_paired_notebook(old_path)
            self.update_paired_notebooks(new_path, fmt, formats)

    return _TextFileContentsManager


TextFileContentsManager = create_text_file_contents_manager(LargeFileManager)
